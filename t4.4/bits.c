 ...
 *
 */
uint32_t op_bit_set(uint32_t data, int i) { return data | (1 << i); }
...
uint32_t op_bit_clear(uint32_t data, int i) { return data & (~0 ^ (1 << i)); }
...
uint32_t op_bit_toggle(uint32_t data, int i) { return data ^ (1 << i); }
...
int op_bit_get(uint32_t data, int i) { return (data & (1 << i)) ? 1 : 0; }
...
void op_print_byte(unsigned char b) { printf("%08b", b); }
